...
const service = {
  ...
  exerciseSessions: () => FIND_MANY_RESULT,
  exerciseSession: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("ExerciseSession", () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        {
          provide: ExerciseSessionService,
          useValue: service,
        },
      ],
  ...
  test("POST /exerciseSessions", async () => {
    await request(app.getHttpServer())
      .post("/exerciseSessions")
      .send(CREATE_INPUT)
  ...
  test("GET /exerciseSessions", async () => {
    await request(app.getHttpServer())
  ...
  test("GET /exerciseSessions/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/exerciseSessions"}/${nonExistingId}`)
  ...
  test("GET /exerciseSessions/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/exerciseSessions"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /exerciseSessions existing resource", async () => {
